...
public class ActionsHandler {
    ...
    public static void ActionRunner(SwerveDriveTrain driveTrain, Motor shouldeMotor, Motor armExtension) {
        try {
            ...
            Thread t1 = new Thread(new actions(driveTrain, shouldeMotor, armExtension, "driveRotate", Math.PI - 0.01));
            Thread t2 = new Thread(new actions(driveTrain, shouldeMotor, armExtension, "driveDistance", 0.21));
            Thread t3 = new Thread(new actions(driveTrain, shouldeMotor, armExtension, "placeThingy", 1));
            Thread t4 = new Thread(new actions(driveTrain, shouldeMotor, armExtension, "DriveBackwardsThenRotate", 10));
